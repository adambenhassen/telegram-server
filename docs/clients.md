...
Two things sit outside that, on purpose. A connection carrying no client address
is charged to no network bucket, so keep `TG_CLIENT_ADDR_PROXY_CIDRS` to the
balancers. And the bounds end at the first frame that decrypts under a key the
server issued, not at sign-in: a client waiting on a human reading a login code
must not be closed, so one completed key exchange buys connections these three
settings no longer count. Bounding what a single not-yet-signed-in key may hold
is separate state and a separate setting, and is not shipped here.
