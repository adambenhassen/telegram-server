...
type AuthKeyStore interface {
	...
	// Get returns the auth key and the user bound to it (0 when unbound) in a
	// single lookup; ok is false when the key is absent.
	Get(ctx context.Context, id [8]byte) (key crypto.AuthKey, userID int64, ok bool, err error)
...
// user bindings, so userID is always 0.
func (s *memoryAuthKeyStore) Get(_ context.Context, id [8]byte) (crypto.AuthKey, int64, bool, error) {
	s.mu.Lock()
	key, ok := s.keys[id]
	s.mu.Unlock()
	return key, 0, ok, nil
