...
// sentinels track the latest migration (auth_keys table + phone_codes.attempts);
// update them when a migration adds new schema.
func (s *Store) checkSchema(ctx context.Context) error {
	var hasAuthKeys, hasHardening bool
	err := s.pool.QueryRow(ctx, `
		SELECT to_regclass('public.auth_keys') IS NOT NULL,
		       EXISTS(SELECT 1 FROM information_schema.columns
		              WHERE table_name = 'phone_codes' AND column_name = 'attempts')`,
	).Scan(&hasAuthKeys, &hasHardening)
	if err != nil {
		return fmt.Errorf("schema check: %w", err)
	}
	if !hasAuthKeys || !hasHardening {
