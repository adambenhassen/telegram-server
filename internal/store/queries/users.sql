...
WHERE u.name_tsv @@ plainto_tsquery('simple', sqlc.arg(query))
  AND EXISTS (
      ...
      WHERE (d.owner_id = sqlc.arg(owner_id)::bigint AND d.peer_id = u.id AND d.peer_type = 1)
         OR (d.peer_id = sqlc.arg(owner_id)::bigint AND d.owner_id = u.id AND d.peer_type = 1)
  )
