...
#
# A daemon reached over DOCKER_HOST rather than a bind-mounted socket — a DinD
# sidecar — has never heard of this container, so it answers "No such
...
#
# Otherwise only "already in the network" is ignored, and nothing else: a silent
# failure here buys back the exact timeout this target exists to prevent,
# minutes later and looking like a broken test. /etc/hostname is not the
# container's Docker name under --hostname or pod-style networking, and the
# daemon may be unreachable — both must be loud. A container already on the
# bridge keeps whatever priority it joined with; `docker network disconnect
# bridge <name>` once, then rerun.
docker-bridge:
	@[ -f /.dockerenv ] || exit 0; \
	...
	*"No such container"*) \
	   echo "make: docker daemon at $${DOCKER_HOST:-the local socket} does not manage this container, so there is no bridge to join; pgtest will reach Postgres on its published port. See docs/testing.md" >&2; \
	   exit 0;; \
